...
@section('title', 'Commandes clients - Olten')
...
@php
    ...
    $toProcess  = $onPage->whereIn('order_status', ['pending', 'confirmed'])->count();
    $delivered  = $onPage->where('order_status', 'delivered')->count();
    $revenue    = $onPage->filter(fn ($o) => $o->order_status !== 'cancelled')
...
<div class="sp-page">
    ...
    <nav class="sp-crumbs" aria-label="Fil d'ariane">
        ...
        <span class="is-current">Commandes clients</span>
    </nav>

    {{-- En-tete --}}
    <header class="sp-head">
        <div>
            <h1 class="sp-title">Commandes clients</h1>
            <p class="sp-subtitle">Acceptez, suivez et gérez les commandes passées sur vos produits.</p>
        </div>

        <a href="{{ route('seller.sales') }}" class="sp-btn-primary">
            Mes ventes
        </a>
    </header>
    ...
    <div class="sp-stats">
        ...
        <div class="sp-stat">
            ...
            <div>
                <span class="sp-stat-value">{{ $toProcess }}</span>
    ...
    <section class="sp-panel">
        ...
        <div class="sp-toolbar">
            ...
            <div class="sp-toolbar-actions">
                <form method="GET" action="{{ route('seller.clientOrders') }}" class="sp-search" role="search">
                    @if($status)
                    ...
                    <div class="sp-search-field">
                        ...
                        @if($search)
                            <a href="{{ route('seller.clientOrders', array_filter(['status' => $status])) }}"
        ...
        <div class="sp-tabs">
            @foreach($tabs as $value => $label)
                <a href="{{ route('seller.clientOrders', array_filter(['search' => $search, 'status' => $value])) }}"
            ...
            <div class="sp-rows">
                @foreach($orders as $order)
                    @php
                        ...
                        $buyer     = $order->buyer;
                        $canCancel = ! in_array($order->order_status, ['delivered', 'cancelled']);
                        $canAccept = $order->order_status === 'pending' && $order->status === 'paid';
                    @endphp
                    ...
                    <article class="sp-row {{ $order->order_status === 'cancelled' ? 'is-cancelled' : '' }}">
                        ...
                        <div class="sp-row-main">
                            <div class="sp-row-head">
                                <h3 class="sp-row-title">{{ $product->name ?? 'Produit supprimé' }}</h3>
                            ...
                            <div class="sp-row-actions">
                                @if($canAccept)
                                    <form action="{{ route('seller.orders.confirm', $order->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="sp-act is-success">
                                            Accepter
                                        </button>
                                    </form>
                                @endif
                                ...
                                @if($order->delivery_requested)
                                    <a href="{{ route('orders.show', $order->id) }}" class="sp-act is-ghost" title="Suivre la commande">
                                        Suivre
                                    </a>
                                @endif
                                ...
                                @if($canCancel)
                                    <form action="{{ route('seller.orders.cancel', $order) }}" method="POST"
                                          data-sp-confirm
                                          data-title="Annuler cette commande ?"
                                          data-text="La commande #{{ $order->id }} sera annulée et le stock réapprovisionné."
                                          data-confirm-label="Oui, annuler">
                                        @csrf
                                        <button type="submit" class="sp-act is-cancel">
                                            Annuler
                                        </button>
                                    </form>
                                @endif
            ...
            <div class="sp-empty">
                @if($search || $status)
                    <x-empty-state
                        ...
                        :action-url="route('seller.clientOrders')"
                        action-label="Voir toutes les commandes" />
                @else
                    <x-empty-state
